...
# Input image of laser dot. Using Contours, determine the (x,y) pixel coordinates of each laser dot in the image.
def findpix(im):
    # TODO: Look for green, not red
    ret,bim = cv2.threshold(im[:,:,2],240,255,cv2.THRESH_BINARY);
    ...
    # TODO: Find a range to filter out objects that obviously arent a laser
...
def finddist(left_cam_x,right_cam_x):
        ...
        l_1 = abs(right_cam_x - l_0);   # dist from laser to image center
        theta_1 = math.pi/2 - math.atan(math.tan(theta_0)*(l_1/l_0));

        l_2 = abs(left_cam_x - l_0);    # dist from laser to image center
